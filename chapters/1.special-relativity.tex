...
Since proper time $\dd\tau$ is an \emph{invariant}, we are motivated to define the 4-vector
\begin{align}
	u^\mu \equiv \dv{x^\mu}{\tau}
	= \dv{t}{\tau}\qty(c, \dv{\uvec x}{t})
	= \gamma(c, \uvec v)
,\end{align}
where $\gamma$ is defined as in \eqref{eqn:gamma-factor}.
In the particle's instantaneous rest frame, $\|\uvec v\|^2 = u^iu_i = 0$, so that $u^\mu u_\mu = u^0u_0 = -c^2$.
However, since this is an invariant (it is a scalar, which does not transform under Poincaré transformations), we have
\begin{eqbox}
	\|\ts u\|^2 \equiv \ts u \cdot \ts u \equiv u^\mu u_\mu = -c^2
\end{eqbox}
in \emph{all frames}.
...
The action is
\begin{align}
	S = -mc^2\int_{\ts x_i}^{\ts x_f} \dd\tau = \int_{t_i}^{t_f} \dd t L
\end{align}
...
\begin{align}
	E &= \uvec p\cdot\uvec x - L = \gamma mv^2 + \frac{mc^2}{\gamma} = \gamma mc^2
...
In all rest frames,
\begin{eqbox}
	\|\ts p\|^2 = \ts p\cdot\ts p = p^\mu p_\mu = -m^2c^2
\end{eqbox}
...
The covariant orbital angular momentum of a particle about an event $\ts x_0 = \qty{x_0^\mu}$ is defined as
\begin{align}
...
\begin{note}[Example]
	About the origin $x_0^μ = (0,0,0,0)$, we have
	\begin{math}
		j^{ij} = x^ip^j - x^jp^i
	\end{math}
...
\section{Gau\ss's Theorem in Minkowski Spacetime}