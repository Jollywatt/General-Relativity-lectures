...
\begin{itemize}
	...
	In terms of the velocity $\beta \equiv \frac{v}{c}$, one has $\coshα = \frac1{\sqrt{1 - \beta^2}} \equiv γ$ and $\sinhα = \frac{\beta}{\sqrt{1 - \beta^2}} \equiv \betaγ$.
...
\noindent
An important difference between rotations and boosts is that, where $0 \le θ < 2π$ for rotations, we have $-\infty < α < \infty$ for boosts, i.e., rotations form a \textit{compact}\footnote{A \textit{compact} set is one for which any infinite sequence of elements contains a convergent subsequence. E.g., $[0, 2π)$ is compact, but $\mathbb{R}$ is not (consider the sequence $\qty{1, 2, 3, ...} \subset \mathbb{R}$).} subgroup of the Lorentz (or Poincaré) group, whereas boosts are \textit{non-compact}---and in fact do not form a subgroup (because, in general, the composition of two boosts forms a combination of a rotation and a boost).
